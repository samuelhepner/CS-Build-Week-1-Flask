...
item_list = [Item('Pile of Gold', 'Contains several coins', 20),
             ...
             Food('Root', 'Yum...fiber', 1, 'healthy', 15),
             Food('Green Leaf', 'You need the nutrients', 1, 'healthy', 15),
...
class World:
    ...
    def create_world(self, num_rooms):
        ...
        self.starting_room = Room(levels['name'][rand_num], levels['description'][rand_num], room_count, first_x,
                                  first_y, items=[random.choice(item_list), random.choice(item_list)])
            ...
            if direction == 1:
                ...
                if self.height > (y + 1) > -1:
                    if self.grid[y + 1][x] is None:
                        ...
                        direction_list = [1, 2, 3, 4]
                        rand_num = random.randint(0, 12)
                        room = Room(levels['name'][rand_num], levels['description'][rand_num], room_count, x, y,
                                    items=[random.choice(item_list), random.choice(item_list)])
        ...
        for room in rooms_list:
            ...
            if (exits == ['e', 'n', 's', 'w'] or
                    exits == ['e', 's'] or exits == ['e', 'n'] or
                    exits == ['s', 'w'] or exits == ['n', 'w']):
                room.store = Store(stock=[random.choice(item_list), random.choice(item_list)],
                                   vault=random.randint(50, 100))
                if exits == ['e', 'n', 's', 'w']:
